...
class Feat_map(object):
	...
	def define_patches(self):
		...
		for p in range(num_patches):
			...
			patch = Patch(self.name, p, value, self.ellipse[p], [self.centerx[p], self.centery[p]], 
	...
	def define_patches_v2(self, value, expVal):
		...
		for p in range(num_patches):
			...
			patch = Patch(self.name, p, value,  self.ellipse[p], [self.centerx[p], self.centery[p]], 
						 [self.radius1[p], self.radius2[p]], self.theta[p], np.pi * (self.radius1[p]/2) * (self.radius2[p]/2), 
						 self.boundaries[p], expVal = expVal, diag_size=diag_size, total_area=total_area)
